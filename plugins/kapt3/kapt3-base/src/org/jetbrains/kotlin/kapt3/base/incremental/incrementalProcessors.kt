...
internal class IncrementalFiler(private val filer: Filer) : Filer by filer {
    ...
    override fun createClassFile(name: CharSequence, vararg originatingElements: Element?): JavaFileObject {
        ...
        println("Generating class file: " + name)
...
internal class AnnotationProcessorDependencyCollector(
    ...
    private fun validateAnnotations(annotations: Set<TypeElement>) {
        for (annotation in annotations) {
            val retention = annotation.getAnnotation(Retention::class.java)
            if (retention != null && retention.value == RetentionPolicy.SOURCE) {
                isFullRebuild = true
                warningCollector.invoke("'@" + annotation.simpleName + "' has source retention. Aggregating annotation processors require class or runtime retention")
            }
        }
    }
